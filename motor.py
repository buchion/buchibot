...
class MOTOR:
    ...
    def Set_Value(self, robotId, targetPosition): 
        ...
        joint_name_bytes = self.jointName.encode("utf-8") if isinstance(self.jointName, str) else self.jointName

        if joint_name_bytes not in pyrosim.jointNamesToIndices:
            raise KeyError(f"Joint name '{self.jointName}' not found in pyrosim.jointNamesToIndices. "
                        f"Available keys: {list(pyrosim.jointNamesToIndices.keys())}")
